...
char* gera_numero(size_t max) {
    size_t size = rand() % max + 1; // tamanho vai receber um o resto da divisao de  um numero randomico pelo max +1
    char* num = (char*) calloc(size, sizeof(char)); // aloca um espaço em memoria inicializando o array com zero
    num[0] = rand() % 2? '-': '+'; // verifica se a posicao gerada pelo numero randomico eh divisivel por 2, se o for seu sinal sera - do contratrio +
    do {
        num[1] = rand() % 10 + '0';
    } while (num[1] == '0');
...
int main(int argc, char* argv[]) {
    ...
    if (0 == max) {
        ...
        num1 = gera_numero(max); // gera um numero aleatorio apartir do parametro informado
